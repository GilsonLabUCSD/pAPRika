...
class DAT_restraint(object):
    ...
    def __init__(self):
        ...
        self.attach = {
            ...
            'fraction_increment':
            None,  # The percentage of the force constant increment (optional)
            'fraction_list':
            None,  # The list of force constant percentages (optional)
            'fc_list':
            None  # The list of force constants (will be created if not given)
        }
        ...
        self.pull = {
            ...
            'fraction_increment':
            None,  # The percentage of the target value increment (optional)
            'fraction_list':
            None,  # The list of target value percentages (optional)
            'target_list':
            None  # The list of target values (will be created if not given)
        }
        ...
        self.release = {
            ...
            None,  # The list of force constant percentages (optional)
            'fc_list':
            None  # The list of force constants (will be created if not
            # given)
        }
    ...
    def _calc_meth(self, phase, rdict, meth):
        ...
        if phase == 'p' and meth == '1':
            targets = np.linspace(rdict['target_initial'],
                                  rdict['target_final'], rdict['num_windows'])

        # Pull, Target Method 1a
        elif phase == 'p' and meth == '1a':
            targets = np.linspace(0.0, rdict['target_final'],
                                  rdict['num_windows'])
        ...
        # Pull, Target Method 2a
            ...
            targets = np.arange(
                0.0, rdict['target_final'] + rdict['target_increment'],
                rdict['target_increment'])
    ...
    def initialize(self):
        ...
        if self.attach['num_windows'] is not None and self.attach['fc_final'] is not None:
            if self.attach['fc_initial'] is not None:
                ...
                force_constants, targets = self._calc_meth(
                    'a', self.attach, '1')
            else:
                ### METHOD 1a ###
        ...
        if self.auto_apr and self.release['fc_final'] is not None:
            ...
            for key in [
                    ...
                    'fraction_increment', 'fraction_list', 'fc_list'
            ]:
        ...
        for phase in ['attach', 'pull', 'release']:
            if self.phase[phase]['targets'] is not None:
                ...
                log.debug('Number of {} windows = {}'.format(
                    phase, window_count))
            else:
...
def check_restraints(restraint_list, create_window_list=False):
    ...
    if all(restraint.continuous_apr is True for restraint in restraint_list):
        ...
        all_continuous_apr = True
            ...
            restraint.continuous_apr is False for restraint in restraint_list):
    ...
    for phase in phases:
        ...
        if all(count == 0 or count == max_count for count in win_counts):
            ...
            log.error(
                'Restraints have unequal number of windows during the {} phase.'.
                format(phase))
...
def amber_restraint_line(restraint, phase, window):
    ...
    for key, value in restraint.custom_restraint_values.items():
        ...
        if value is not None:
            ...
            restraint.amber_restraint_values[key] = value
    ...
    if any([
            restraint.group1, restraint.group2, restraint.group3,
            restraint.group4
    ]):
        string += '\n    '
        if restraint.group1:
    ...
    string += "  &end\n"
    return string, amber_restraint_line
