...
class Setup(object):
    ...
    def add_dummy_atoms(self, PDB_filename="output.pdb", XML_filename="system.xml"):
        # First add dummy atoms to structure
        guest_angle_restraint_mask = self.guest_yaml["restraints"][-1]["restraint"]["atoms"].split()
        for window in self.window_list:
            logger.debug(f"Adding dummy atoms to {PDB_filename}")
            try:

                file_path = self.directory.joinpath("windows").joinpath(window).joinpath(
                    PDB_filename)

                structure = pmd.load_file(str(file_path))
                offset_coordinates = structure[guest_angle_restraint_mask[1]].coordinates
                input_pdb_file = openmm.app.PDBFile(str(file_path))

                positions = input_pdb_file.positions
                positions.append(openmm.Vec3(offset_coordinates[0][0], offset_coordinates[0][1], offset_coordinates[0][2] - 6.0) * unit.angstrom)
                positions.append(openmm.Vec3(offset_coordinates[0][0], offset_coordinates[0][1], offset_coordinates[0][2] - 9.0) * unit.angstrom)
                positions.append(openmm.Vec3(offset_coordinates[0][0], offset_coordinates[0][1] + 2.2, offset_coordinates[0][2] - 11.2) * unit.angstrom)
                ...
                logger.warning(f"Missing {PDB_filename} in {window}")
            ...
            try:
                ...
                with open(self.directory.joinpath("windows").joinpath(window).joinpath(XML_filename), "w") as file:
                    file.write(system_xml)
            except:
                logger.warning(f"Missing {XML_filename}")
    ...
    def initialize_restraints(self, PDB_filename="output.pdb"):
        ...
        structure = self.directory.joinpath("windows").joinpath("a000").joinpath(PDB_filename)
...
def apply_openmm_restraints(system, restraint, window, ForceGroup=None):
    ...
    if restraint.mask2 and not restraint.mask3:
        if not restraint.group1 and not restraint.group2:
            ...
            bond_restraint = openmm.CustomCentroidBondForce(2, 'k * (distance(g1, g2) - r_0)^2')
            bond_restraint.addPerBondParameter('k')
            bond_restraint.addPerBondParameter('r_0')
            r_0 = restraint.phase[phase]["targets"][window_number] * unit.angstroms
            k = restraint.phase[phase]["force_constants"][window_number] * unit.kilocalories_per_mole / \
