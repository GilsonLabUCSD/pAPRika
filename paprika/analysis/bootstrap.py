...
def integrate_bootstraps(x, ys, x_intp=None, matrix="full"):
    ...
    x_intp: :class:`numpy.array`, optional, default=None
    ...
    x_idxs = numpy.zeros([num_x], numpy.int32)
    ...
    # array.
    if x_intp is None:
        x_intp = numpy.zeros([0], numpy.float64)
        for i in range(1, num_x):
            x_intp = numpy.append(
                x_intp, numpy.linspace(x[i - 1], x[i], num=100, endpoint=False)
            )
            x_idxs = len(x_intp)
        # Tack on the final value onto the interpolation
        x_intp = numpy.append(x_intp, x[-1])
    # If x_intp is provided, find the locations of x values in x_intp
    else:
        i = 0
        for j in range(len(x_intp)):
            if numpy.isclose(x[i], x_intp[j]):
                x_idxs[i] = j
                i += 1
        if i != num_x:
            raise Exception(
    ...
    int_matrix = numpy.zeros([num_x, num_x, cycles], numpy.float64)
    ...
    if matrix == "endpoints":
        for cycle in range(cycles):
            ...
            #                    int_matrix[i, j, cycle] = numpy.trapz( y_intp, x_intp )
            int_matrix[0, num_x - 1, cycle] = numpy.trapz(y_intp, x_intp)
    else:
        for cycle in range(cycles):
            intp_func = Akima1DInterpolator(x, ys[cycle])
            y_intp = intp_func(x_intp)
            for i in range(0, num_x):
                for j in range(i + 1, num_x):
                    ...
                    int_matrix[i, j, cycle] = numpy.trapz(
    ...
    avg_matrix = numpy.zeros([num_x, num_x], numpy.float64)
    sem_matrix = numpy.zeros([num_x, num_x], numpy.float64)
    ...
    for i in range(0, num_x):
        for j in range(i + 1, num_x):
            ...
            if matrix == "endpoints" and i != 0 and j != num_x - 1:
                continue
            avg_matrix[i, j] = numpy.mean(int_matrix[i, j])
            avg_matrix[j, i] = -1.0 * avg_matrix[i, j]
            sem_matrix[i, j] = numpy.std(int_matrix[i, j])
...
def regression_bootstrap(
    ...
    summary_statistics = numpy.empty((cycles, 8))

    # Bootstrapping
    for cycle in range(cycles):
        new_x = numpy.empty_like(x_data)
        new_y = numpy.empty_like(y_data)
        for index in range(len(x_data)):
            if with_replacement:
                j = numpy.random.randint(len(x_data))
            else:
                j = index
            if with_uncertainty and x_sem is not None:
                new_x[index] = numpy.random.normal(x_data[j], x_sem[j])
            elif with_uncertainty and x_sem is None:
                new_x[index] = x_data[j]
            if with_uncertainty and y_sem is not None:
                new_y[index] = numpy.random.normal(y_data[j], y_sem[j])
    ...
    for statistic in range(8):
        sorted_statistic = numpy.sort(summary_statistics[:, statistic])
    ...
    results = {
        "mean": {
            "slope": numpy.mean(summary_statistics[:, 0]),
            "intercept": numpy.mean(summary_statistics[:, 1]),
            "R": numpy.mean(summary_statistics[:, 2]),
            "R**2": numpy.mean(summary_statistics[:, 3]),
            "RMSE": numpy.mean(summary_statistics[:, 4]),
            "MSE": numpy.mean(summary_statistics[:, 5]),
            "MUE": numpy.mean(summary_statistics[:, 6]),
            "Tau": numpy.mean(summary_statistics[:, 7]),
        },
        "sem": {
            "slope": numpy.std(summary_statistics[:, 0]),
            "intercept": numpy.std(summary_statistics[:, 1]),
            "R": numpy.std(summary_statistics[:, 2]),
            "R**2": numpy.std(summary_statistics[:, 3]),
            "RMSE": numpy.std(summary_statistics[:, 4]),
            "MSE": numpy.std(summary_statistics[:, 5]),
            "MUE": numpy.std(summary_statistics[:, 6]),
            "Tau": numpy.std(summary_statistics[:, 7]),
        },
        "ci_low": {
            "slope": ci[0][0],
            "intercept": ci[1][0],
...
def dG_bootstrap(
    x_data: Union[List, numpy.array],
    x_sem: Union[List, numpy.array],
    ...
    for cycle in range(cycles):
        ...
        if with_uncertainty and x_sem is not None:
            new_x = numpy.random.normal(x_data, x_sem)
        elif with_uncertainty and x_sem is None:
            new_x = x_data
        if with_uncertainty and y_sem is not None:
            new_y = numpy.random.normal(y_data, y_sem)
        elif with_uncertainty and y_sem is None:
            new_y = y_data
        summary_statistics[cycle] = -RT * numpy.log(
            numpy.exp(-beta * new_x) + numpy.exp(-beta * new_y)
        )

        # Get confidence interval
        sorted_statistic = numpy.sort(summary_statistics)
        ci[0] = sorted_statistic[int(0.025 * cycles)]
        ci[1] = sorted_statistic[int(0.975 * cycles)]

    results = {
        "mean": numpy.mean(summary_statistics),
        "sem": numpy.std(summary_statistics),
        "ci": ci,
    }
...
def dH_bootstrap(
    ...
    ci = numpy.empty((2))
    for cycle in range(cycles):
        new_dH_x = numpy.empty_like(dH_x_data)
        new_dH_y = numpy.empty_like(dH_y_data)

        new_dG_x = numpy.empty_like(dG_x_data)
        new_dG_y = numpy.empty_like(dG_y_data)

        # Resample dH
        if with_uncertainty and dH_x_sem is not None:
            new_dH_x = numpy.random.normal(dH_x_data, dH_x_sem)
        elif with_uncertainty and dH_x_sem is None:
            new_dH_x = dH_x_data
        if with_uncertainty and dH_y_sem is not None:
            new_dH_y = numpy.random.normal(dH_y_data, dH_y_sem)
        elif with_uncertainty and dH_y_sem is None:
            new_dH_y = dH_y_data

        # Resample dG
        if with_uncertainty and dG_x_sem is not None:
            new_dG_x = numpy.random.normal(dG_x_data, dG_x_sem)
        elif with_uncertainty and dG_x_sem is None:
            new_dG_x = dG_x_data
        if with_uncertainty and dG_y_sem is not None:
            new_dG_y = numpy.random.normal(dG_y_data, dG_y_sem)
        elif with_uncertainty and dG_y_sem is None:
            new_dG_y = dG_y_data

        summary_statistics[cycle] = (
            new_dH_x * numpy.exp(-beta * new_dG_x)
            + new_dH_y * numpy.exp(-beta * new_dG_y)
        ) / (numpy.exp(-beta * new_dG_x) + numpy.exp(-beta * new_dG_y))
...
def summarize_statistics(x, y):
    ...
    summary_statistics: List
        ...
        * Tau - Kendall's Tau
    """
    summary_statistics = numpy.empty(8)
    ...
    # RMSE - Root-Mean-Squared-Error
    summary_statistics[4] = numpy.sqrt(numpy.mean((y - x) ** 2))

    # MSE - Mean Signed Error
    summary_statistics[5] = numpy.mean(y - x)

    # MUE - Mean Unsigned Error
    summary_statistics[6] = numpy.mean(numpy.absolute(y - x))
